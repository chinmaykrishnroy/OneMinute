...
room.join is a temporary development-only capability flow. The application endpoint `/v1/discovery/ws` authenticates the Go session and exact Origin before accepting a socket. Discovery client events are `queue.join`, `queue.leave`, `presence.heartbeat`, `webrtc.offer`, `webrtc.answer`, `webrtc.ice` and `match.leave`. Server events are `connection.ready`, `queue.joined`, `queue.left`, `match.found`, `peer.disconnected`, `peer.reconnected`, relayed WebRTC events, `match.ended` and `error`.

`queue.join` carries a bounded current intent, one to three language tags and up to eight allowlisted session interests. It never carries a destination user ID. The server derives identity from the session cookie, and every match-scoped event is authorized against Redis user-to-match state and the active socket connection.
