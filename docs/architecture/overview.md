# Architecture
Status: Milestones 0–5 implement networking, identity, distributed discovery, encounters and the social graph. Later features below are architecture decisions, not claims of implementation.
...
- `apps/web`: Next.js App Router / React / TypeScript. Server Components render route shells; Client Components own media, signaling, timers and interactive social screens.
- `apps/server`: Go modular monolith using net/http, pgx and go-redis. It owns application identity, discovery, live encounter authority, durable social state, health/readiness, migrations and the explicitly gated Redis-backed signaling lab.
...
The remaining sequence is persistent communication, intelligent discovery and hardening. See [roadmap](roadmap.md). MinIO will provide private S3-compatible object bytes behind a provider-neutral storage boundary; PostgreSQL stores metadata and references, never media blobs.
