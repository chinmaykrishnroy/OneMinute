# Architecture
Status: Milestone 0 foundation. Later features below are decisions, not claims of implementation.
...
- `apps/server`: Go modular monolith using net/http, pgx and go-redis. Currently health/readiness and migration infrastructure.
...
No SFU, Kubernetes, recording, or persistent chat. Pion WebRTC diagnostics will be a separate optional executable in Milestone 1; normal calls never use a Go PeerConnection.
